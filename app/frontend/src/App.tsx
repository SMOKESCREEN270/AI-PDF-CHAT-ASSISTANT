...
import { MarkdownMessage, CitationMarker } from '@/components/markdown-message';
...
function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const refreshProjects = useCallback(async () => {
    ...
    try {
      ...
      // Same reasoning as refreshChatSessions - never block the rest of the app on this.
    }
  }, [user]);
  ...
  }), [documents, refreshDocuments, selectedId, notify, chatSessions, refreshChatSessions, activeSessionId, projects, refreshProjects]);
...
function PdfViewer({ citation, onClose }: { citation: Citation; onClose: () => void }) {
  ...
  useEffect(() => {
    // Capped lower than before (560 vs 680): scanned/photographed PDF pages
    // from the OCR fallback path are raster images, and PDF.js's canvas
    // backing store scales with width^2 * devicePixelRatio^2 - on a
    // high-DPI (2x/3x) display, a wide canvas for an image-heavy page can
    // by itself use tens of MB, and doing that repeatedly (switching
    // between citations without the tab ever fully releasing the previous
    // canvas) is a well-documented way to hit a PDF.js "Out of Memory" tab
    // crash. A smaller max render width keeps peak canvas memory bounded
    // without materially hurting readability.
    const updateWidth = () => setPageWidth(Math.max(280, Math.min(560, window.innerWidth - 430)));
    updateWidth();
...
function Chat() {
  ...
  // Tracks whether the auto-select-a-default-document effect below has
  // already run once for this brand-new chat. Without this, unchecking the
  // only selected document brings `chosen` back to zero length, which used
  // to make the effect fire again and silently re-select a document out
  // from under you - so it looked like you couldn't deselect a PDF at all
  // unless you first picked a second one. It should only ever pick a
  // starting default once, never "helpfully" re-add one after a deliberate
  // deselect.
  const didAutoSelect = useRef(false);

  // Default to the most recently uploaded ready document for a brand-new chat.
  useEffect(() => {
    if (activeSessionId || didAutoSelect.current) return;
    const firstReady = documents.find((doc) => doc.status === 'ready');
    if (firstReady) {
      didAutoSelect.current = true;
      setChosen([firstReady.id]);
    }
            ...
            {!loadingSession && messages.map((message, index) => <div key={`${message.created_at || index}-${index}`} className={`message ${message.role === 'user' ? 'user' : ''}`}>
              ...
              <div className="summary-copy" style={{ fontSize: 17 }}>
                {message.role === 'assistant'
                  ? <MarkdownMessage content={message.content} citations={message.citations} onOpenCitation={setActiveCitation} />
                  : message.content /* Safe by default: React escapes the user's own message as text. */}
              </div>
              ...
              {message.role === 'assistant' && (message.citations?.length || message.highlighted_sections?.length) ? <div className="source-chip-row">
                <span className="eyebrow">Sources</span>
                {message.citations?.map((citation, citationIndex) => <CitationMarker key={`${citation.document_id}-${citation.page}-${citationIndex}`} index={citationIndex + 1} citation={citation} onOpen={setActiveCitation} />)}
                {message.highlighted_sections?.length ? <>
                  <span className="eyebrow source-chip-divider">Highlighted</span>
                  {message.highlighted_sections.map((citation, citationIndex) => <CitationMarker key={`highlight-${citation.document_id}-${citation.page}-${citationIndex}`} index={citationIndex + 1} citation={citation} onOpen={setActiveCitation} />)}
                </> : null}
              </div> : null}
              {/* Confidence sits at the very end of the message, after the answer, summary, and sources. */}
...
/**
 * Per-row "..." menu on a history/chat row: rename, share via link, add to
 * a project, delete. Kept as its own component so History and Projects can
 * both use it against the same actions.
 */
function ChatRowMenu({ session }: { session: ChatSessionSummary }) {
  const { notify, refreshChatSessions, refreshProjects, projects } = useWorkspace();
  const [open, setOpen] = useState(false);
  const [mode, setMode] = useState<'menu' | 'rename' | 'share' | 'move'>('menu');
  ...
  async function submitRename() {
    ...
    try {
      ...
      setOpen(false);
      setMode('menu');
    } catch (cause) { notify(apiError(cause)); } finally { setBusy(false); }
  }
  ...
  async function openShare() {
    ...
    setBusy(true);
    try {
      const link = await client.shareSession(session.id);
  ...
  async function copyShareLink() {
    try {
      ...
      notify('Could not copy automatically - copy the link manually');
    }
  }
  ...
  async function moveToProject(projectId: string | null) {
    setBusy(true);
    try {
      await client.moveSessionToProject(session.id, projectId);
      await Promise.all([refreshChatSessions(), refreshProjects()]);
      notify(projectId ? 'Added to project' : 'Removed from project');
      setOpen(false);
      setMode('menu');
    } catch (cause) { notify(apiError(cause)); } finally { setBusy(false); }
  }
  ...
  return <div className="row-menu-wrap" ref={wrapRef} onClick={(event) => event.stopPropagation()}>
    <button type="button" className="icon-button" aria-label="Chat options" onClick={toggle}><MoreVertical size={16} /></button>
    {open && <div className="row-menu-popover">
      {mode === 'menu' && <div className="row-menu-list">
        <button type="button" onClick={() => setMode('rename')}><Pencil size={13} /> Rename</button>
        <button type="button" onClick={() => void openShare()}><Share2 size={13} /> Share via link</button>
        <button type="button" onClick={() => setMode('move')}><FolderKanban size={13} /> Add to project</button>
        <button type="button" className="row-menu-danger" onClick={() => void deleteChat()}><Trash2 size={13} /> Delete</button>
      </div>}
      {mode === 'rename' && <div className="row-menu-form">
        <input className="field" value={title} onChange={(event) => setTitle(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void submitRename(); }} autoFocus />
        <div className="row-menu-actions"><button type="button" className="outline-button" onClick={() => setMode('menu')}>Cancel</button><button type="button" className="gold-button" disabled={busy} onClick={() => void submitRename()}>Save</button></div>
      </div>}
      {mode === 'share' && <div className="row-menu-form">
        {busy && !shareUrl ? <p className="small muted">Preparing link…</p> : <>
          <input className="field" readOnly value={shareUrl} onFocus={(event) => event.currentTarget.select()} />
          <div className="row-menu-actions">
            <button type="button" className="outline-button" onClick={() => void revokeShare()} disabled={busy}>Revoke</button>
            <button type="button" className="gold-button" onClick={() => void copyShareLink()}>Copy link</button>
          </div>
        </>}
      </div>}
      {mode === 'move' && <div className="row-menu-form">
        {projects.length === 0
          ? <p className="small muted">No projects yet - create one from the Projects page first.</p>
          : <div className="row-menu-list">
            {projects.map((project) => <button type="button" key={project.id} disabled={busy} onClick={() => void moveToProject(project.id)}>
              <FolderKanban size={13} /> {project.name}{session.project_id === project.id ? ' ✓' : ''}
            </button>)}
            {session.project_id && <button type="button" disabled={busy} onClick={() => void moveToProject(null)}><X size={13} /> Remove from project</button>}
          </div>}
      </div>}
    </div>}
  </div>;
  ...
  return <div className="page">
    ...
    <section className="card card-pad">
      {sorted.length === 0
        ...
        : <div>{sorted.map((session) => <div key={session.id} className="history-row" style={{ cursor: 'pointer' }} onClick={() => resumeChat(session.id)}>
          <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {session.share_token && <span className="kind-badge" title="This chat has an active share link"><Share2 size={11} /> Shared</span>}
            <span className="small muted">{timeAgo(session.created_at)}</span>
            <ChatRowMenu session={session} />
          </div>
        </div>)}</div>}
    </section>
  </div>;
...
function Projects() {
  const { projects, refreshProjects, chatSessions, setActiveSessionId, notify } = useWorkspace();
  const [, setLocation] = useLocation();
  ...
  async function submitRename(projectId: string) {
    ...
    setBusy(true);
    try {
      await client.renameProject(projectId, renameValue.trim());
      await refreshProjects();
      setRenamingId(null);
  ...
  async function removeProject(projectId: string) {
    if (busy) return;
    if (!window.confirm('Delete this project? Chats inside it will stay in your History, just un-filed.')) return;
    setBusy(true);
    try {
  ...
  return <div className="page">
    <div className="page-heading">
      <div><span className="eyebrow">Organize your work</span><h1>Projects.</h1><p>Group related chats into a folder. Limited to {MAX_PROJECTS} projects - the assistant keeps an up-to-date memory of each one as chats are added or removed.</p></div>
      <FolderKanban size={26} color="var(--gold)" />
    </div>
    {projects.length < MAX_PROJECTS && <section className="card card-pad" style={{ marginBottom: 24 }}>
      {creating
        ? <div className="row-menu-form" style={{ maxWidth: 420 }}>
          <input className="field" placeholder="Project name" value={newName} onChange={(event) => setNewName(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void submitCreate(); }} autoFocus />
          <div className="row-menu-actions"><button className="outline-button" onClick={() => { setCreating(false); setNewName(''); }}>Cancel</button><button className="gold-button" disabled={busy} onClick={() => void submitCreate()}>Create</button></div>
        </div>
        : <button className="outline-button" onClick={() => setCreating(true)}><Plus size={15} /> New project</button>}
    </section>}
    {projects.length === 0 && !creating && <div className="card card-pad empty-state"><FolderKanban size={26} /><h3>No projects yet</h3><p>Create a project to group related chats together.</p></div>}
    <div className="grid grid-2">
      {projects.map((project) => {
        const projectChats = chatSessions.filter((session) => session.project_id === project.id);
        const isOpen = openProjectId === project.id;
        return <section key={project.id} className="card card-pad">
          <div className="section-title" style={{ marginBottom: 4 }}>
            {renamingId === project.id
              ? <input className="field" value={renameValue} onChange={(event) => setRenameValue(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') void submitRename(project.id); }} autoFocus />
              : <h2>{project.name}</h2>}
            <div style={{ display: 'flex', gap: 6 }}>
              {renamingId === project.id
                ? <button className="icon-button" onClick={() => void submitRename(project.id)} aria-label="Save name"><Check size={15} /></button>
                : <button className="icon-button" onClick={() => { setRenamingId(project.id); setRenameValue(project.name); }} aria-label="Rename project"><Pencil size={14} /></button>}
              <button className="icon-button" onClick={() => void removeProject(project.id)} aria-label="Delete project"><Trash2 size={14} /></button>
            </div>
          </div>
          <p className="small muted" style={{ marginBottom: 12 }}>{projectChats.length} chat{projectChats.length === 1 ? '' : 's'}{project.memory_updated_at ? ` · memory updated ${timeAgo(project.memory_updated_at)}` : ''}</p>
          {project.memory_summary && <div className="card card-pad" style={{ background: 'var(--surface-2, rgba(0,0,0,0.03))', marginBottom: 14 }}>
            <span className="eyebrow">AI project memory</span>
            <p className="serif small" style={{ marginTop: 6, whiteSpace: 'pre-wrap' }}>{project.memory_summary}</p>
          </div>}
          <button type="button" className="outline-button" style={{ marginBottom: 10 }} onClick={() => setOpenProjectId(isOpen ? null : project.id)}>{isOpen ? 'Hide chats' : 'Show chats'}</button>
          {isOpen && (projectChats.length === 0
            ? <p className="small muted">No chats in this project yet - use "Add to project" from a chat's menu in History.</p>
            : <div>{projectChats.map((session) => <div key={session.id} className="history-row" style={{ cursor: 'pointer' }} onClick={() => resumeChat(session.id)}>
              <div><div className="history-row-title">{session.title || 'Untitled chat'}</div><div className="history-row-meta">{session.document_ids.length} document{session.document_ids.length === 1 ? '' : 's'}</div></div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><span className="small muted">{timeAgo(session.created_at)}</span><ChatRowMenu session={session} /></div>
            </div>)}</div>)}
        </section>;
      })}
    </div>
...
function SharedChatPage() {
  ...
  const token = window.location.pathname.split('/shared/')[1] || params.get('token') || '';
  const [chat, setChat] = useState<{ title: string; created_at: string; messages: ChatMessage[] } | null>(null);
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    let active = true;
    setLoading(true);
    client.getSharedChat(token)
      .then((data) => { if (active) setChat(data); })
      .catch((cause) => { if (active) setError(apiError(cause)); })
      .finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, [token]);
  ...
  return <div className="auth-page">
    <div className="page" style={{ maxWidth: 760, margin: '40px auto', width: '100%' }}>
      <div className="page-heading"><div><span className="eyebrow">Shared conversation</span><h1>{chat?.title || 'Shared chat'}</h1><p>A read-only view shared from PDF Assistant.</p></div></div>
      <section className="card card-pad">
        {loading && <div className="empty-state">Loading shared chat…</div>}
        {error && <div className="empty-state"><CircleHelp size={25} /><p>{error}</p></div>}
        {chat && chat.messages.map((message, index) => <div key={index} className={`message ${message.role === 'user' ? 'user' : ''}`}>
          <div className="small eyebrow" style={{ marginBottom: 8 }}>{message.role === 'user' ? 'User' : 'PDF Assistant'}</div>
          <div className="summary-copy" style={{ fontSize: 17 }}>
            {message.role === 'assistant' ? <MarkdownMessage content={message.content} citations={message.citations} /> : message.content}
          </div>
        </div>)}
      </section>
    </div>
...
function Router() {
  return <Switch>
    ...
    <Route path="/verify-email"><VerifyEmailPage /></Route>
    <Route path="/oauth-callback"><OAuthCallbackPage /></Route>
    <Route path="/shared/:token"><SharedChatPage /></Route>
