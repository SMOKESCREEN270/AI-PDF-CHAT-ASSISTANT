...
export type ShareLink = { share_token: string; share_url: string };
...
export const client = {
  ...
  renameSession(sessionId: string, title: string) {
    return request<{ id: string; title: string }>(`/chat/sessions/${encodeURIComponent(sessionId)}`, {
      method: 'PATCH',
      body: JSON.stringify({ title }),
    });
  },
  deleteSession(sessionId: string) {
    return request<{ status: string }>(`/chat/sessions/${encodeURIComponent(sessionId)}`, { method: 'DELETE' });
  },
  shareSession(sessionId: string) {
    return request<ShareLink>(`/chat/sessions/${encodeURIComponent(sessionId)}/share`, { method: 'POST' });
  },
  unshareSession(sessionId: string) {
    return request<{ status: string }>(`/chat/sessions/${encodeURIComponent(sessionId)}/share`, { method: 'DELETE' });
  },
  moveSessionToProject(sessionId: string, projectId: string | null) {
    return request<{ id: string; project_id: string | null }>(`/chat/sessions/${encodeURIComponent(sessionId)}/project`, {
      method: 'PATCH',
      body: JSON.stringify({ project_id: projectId }),
    });
  },
  getSharedChat(shareToken: string) {
    return request<SharedChat>(`/chat/shared/${encodeURIComponent(shareToken)}`);
  },
  listProjects() {
  ...
  deleteProject(projectId: string) {
    return request<{ status: string }>(`/projects/${encodeURIComponent(projectId)}`, { method: 'DELETE' });
  },
  listProjectSessions(projectId: string) {
    return request<ChatSessionSummary[]>(`/projects/${encodeURIComponent(projectId)}/sessions`);
  },
