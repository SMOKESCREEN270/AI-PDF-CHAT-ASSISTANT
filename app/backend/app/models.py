...
class Project(Base):
    """A folder that groups chat sessions together. Limited to 2 per user
    (enforced in the router, not here, so the limit stays easy to change).

    `memory_summary` is an AI-maintained rolling summary of everything
    discussed across every chat currently in this project - it's
    regenerated (see services/memory.update_project_memory) whenever a chat
    is added to or removed from the project, so the assistant always has an
    up-to-date picture of the project as a whole, not just one chat at a time.
    """
