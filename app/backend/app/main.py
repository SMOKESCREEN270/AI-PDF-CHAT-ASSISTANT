...
from app.routers import auth, documents, chat, study_tools, compare, export, projects
...
@app.get("/health")
def health_root():
    """Root-level health check for deployment platforms that probe `/health`
    by default (rather than `/api/health`)."""
    return {"status": "ok", "app": settings.APP_NAME}
