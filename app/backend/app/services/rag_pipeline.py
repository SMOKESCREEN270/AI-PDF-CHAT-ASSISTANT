...
SYSTEM_INSTRUCTION = (
    ...
    "5. Format the answer in Markdown. Use a Markdown table (with a header "
    "row and | separators) whenever the user asks to compare, contrast, or "
    "list things side by side. Use headings, bold, and bullet/numbered "
    "lists to structure longer answers instead of one dense paragraph."
)
...
def renumber_citation_markers(answer_text: str, cited_numbers: List[int]) -> str:
    """
    Rewrites the model's [S<n>] source markers so they line up with the
    1-based position of that source within the final `citations` list
    (which is built from `used_chunks`, in the same order as
    `cited_numbers`), e.g. [S3] -> [2] if source 3 is the second entry in
    `cited_numbers`. Markers referencing a source that isn't in
    `cited_numbers` are dropped, matching the previous strip-all-markers
    behavior for anything the pipeline can't resolve.
    """
    source_to_citation_index = {n: i + 1 for i, n in enumerate(cited_numbers)}

    def _replace(match: "re.Match[str]") -> str:
        new_index = source_to_citation_index.get(int(match.group(1)))
        return f"[{new_index}]" if new_index else ""

    return re.sub(r"\[S(\d+)\]", _replace, answer_text)
...
def run_rag_turn(db: Session, document_ids: List[str], question: str,
    ...
    valid_cited_numbers = [n for n in cited_numbers if 0 < n <= len(retrieved)]
    used_chunks = [retrieved[n - 1] for n in valid_cited_numbers] or retrieved
    ...
    # Renumber the model's [S1], [S2]... markers into sequential [1], [2]...
    # that line up 1:1 with the `citations` array position, instead of
    # stripping them out entirely. The frontend turns each surviving `[n]`
    # marker into a small hover-preview footnote (Wikipedia-style) right at
    # the point in the answer where that source was actually used, rather
    # than only listing sources in a separate block at the end.
    cited_answer = renumber_citation_markers(raw_answer, valid_cited_numbers)
    display_answer = (banner + cited_answer).strip()
