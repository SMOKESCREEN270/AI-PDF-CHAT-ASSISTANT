...
PROJECT_MEMORY_SYSTEM_INSTRUCTION = (
    "You maintain a compact, factual memory for a research project that "
    "spans several chat conversations. Preserve facts, decisions, "
    "recurring themes, and open questions that would help someone pick up "
    "the project later. Do not invent information. Return only a concise "
    "plain-text memory block, without headings or commentary."
)


def update_project_memory(db: Session, project: "models.Project", api_key: str = None) -> None:
    """Regenerates a project's AI memory from the rolling summary (or, for
    short chats, the recent turns) of every chat session currently filed
    under it. Called whenever a chat is added to or removed from a project
    so the project's memory never drifts from what's actually in it.

    Safe to call on an empty project (clears the memory) and never raises -
    a failed memory refresh should never break the add/remove action that
    triggered it.
    """
    sessions = (
        db.query(models.ChatSession)
    ...
    prompt = (
        "Here are the chats currently in this project:\n\n"
        + "\n\n".join(parts)
        + "\n\nWrite the project's updated memory block."
    )
    try:
        summary = llm.generate(
            prompt,
            api_key=api_key,
            system_instruction=PROJECT_MEMORY_SYSTEM_INSTRUCTION,
            temperature=0.1,
        ).strip()
    except Exception:
        # Keep whatever memory already existed rather than wiping it out
        # just because this particular refresh call failed (e.g. rate limit).
        return

    if summary:
        project.memory_summary = summary
        project.memory_updated_at = datetime.utcnow()
        db.add(project)
