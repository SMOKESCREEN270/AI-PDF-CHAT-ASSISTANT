...
def _make_session(owner_id, title="Untitled", project_id=None):
    with SessionLocal() as db:
        session = models.ChatSession(owner_id=owner_id, title=title, document_ids=[], project_id=project_id)
        db.add(session)
        db.commit()
        db.refresh(session)
        return session.id
...
def test_project_creation_is_capped_at_two(client):
    register(client)
    headers = login(client)
    first = client.post("/api/projects", headers=headers, json={"name": "Thesis"})
    assert first.status_code == 200
    second = client.post("/api/projects", headers=headers, json={"name": "Side project"})
    assert second.status_code == 200
    third = client.post("/api/projects", headers=headers, json={"name": "One too many"})
    assert third.status_code == 400


def test_deleting_a_chat_session_removes_it_and_its_messages_from_the_database(client):
    user = register(client)
    headers = login(client)
    session_id = _make_session(user["id"])
    with SessionLocal() as db:
        db.add(models.ChatMessage(session_id=session_id, role="user", content="hi"))
        db.commit()
    ...
    with SessionLocal() as db:
        assert db.query(models.ChatSession).filter(models.ChatSession.id == session_id).first() is None
        assert db.query(models.ChatMessage).filter(models.ChatMessage.session_id == session_id).count() == 0


def test_rename_session_updates_title(client):
    user = register(client)
    headers = login(client)
    session_id = _make_session(user["id"])
    response = client.patch(f"/api/chat/sessions/{session_id}", headers=headers, json={"title": "Renamed chat"})
    assert response.status_code == 200
    assert response.json()["title"] == "Renamed chat"


def test_share_link_exposes_messages_without_auth_and_can_be_revoked(client):
    user = register(client)
    headers = login(client)
    session_id = _make_session(user["id"], title="Shared research chat")
    with SessionLocal() as db:
        db.add(models.ChatMessage(session_id=session_id, role="user", content="What is the thesis about?"))
        db.commit()

    share = client.post(f"/api/chat/sessions/{session_id}/share", headers=headers)
    assert share.status_code == 200
    token = share.json()["share_token"]

    # No Authorization header - this must work for anyone holding the link.
    public = client.get(f"/api/chat/shared/{token}")
    assert public.status_code == 200
    assert public.json()["title"] == "Shared research chat"
    assert len(public.json()["messages"]) == 1

    revoke = client.delete(f"/api/chat/sessions/{session_id}/share", headers=headers)
    assert revoke.status_code == 200
    now_gone = client.get(f"/api/chat/shared/{token}")
    assert now_gone.status_code == 404


def test_moving_a_session_into_and_out_of_a_project_updates_membership(client):
    user = register(client)
    headers = login(client)
    project = client.post("/api/projects", headers=headers, json={"name": "Folder A"}).json()
    session_id = _make_session(user["id"])

    add = client.patch(f"/api/chat/sessions/{session_id}/project", headers=headers,
                        json={"project_id": project["id"]})
    assert add.status_code == 200
    assert add.json()["project_id"] == project["id"]

    listed = client.get(f"/api/projects/{project['id']}/sessions", headers=headers)
    assert listed.status_code == 200
    assert any(s["id"] == session_id for s in listed.json())

    remove = client.patch(f"/api/chat/sessions/{session_id}/project", headers=headers, json={"project_id": None})
    assert remove.status_code == 200
    assert remove.json()["project_id"] is None


def test_deleting_a_project_unfiles_its_chats_instead_of_deleting_them(client):
    user = register(client)
    headers = login(client)
    project = client.post("/api/projects", headers=headers, json={"name": "Folder B"}).json()
    session_id = _make_session(user["id"], project_id=project["id"])
